...
var zHelp = *(*[2632]int8)(unsafe.Pointer(ts))
...
var zDots = *(*[72]int8)(unsafe.Pointer(ts + 2929))
	...
	for sqlite3.Xsqlite3_step(tls, g.pStmt) == SQLITE_ROW {
		...
		for i = 0; i < n; i++ {
			...
			if g.bVerify != 0 {
				...
				*(*uint8)(unsafe.Pointer(bp + 8 + 1)) = uint8(*(*int8)(unsafe.Pointer(ts + 3270 + uintptr(eType))))
				if g.nResByte != 0 {
				...
				if eType == SQLITE_FLOAT {
					*(*uint64)(unsafe.Pointer(uintptr(unsafe.Pointer(&g)) + 96)) += uint64(2)
					...
					for iBlob = 0; iBlob < nBlob; iBlob++ {
						*(*uint8)(unsafe.Pointer(bp + 10)) = uint8(*(*int8)(unsafe.Pointer(ts + 3277 + uintptr(int32(*(*uint8)(unsafe.Pointer(aBlob + uintptr(iBlob))))>>4))))
						*(*uint8)(unsafe.Pointer(bp + 10 + 1)) = uint8(*(*int8)(unsafe.Pointer(ts + 3277 + uintptr(int32(*(*uint8)(unsafe.Pointer(aBlob + uintptr(iBlob))))&15))))
			...
			if uint32(g.nResult+len) < uint32(unsafe.Sizeof([3000]int8{}))-uint32(2) {
				if g.nResult > 0 {
					*(*int8)(unsafe.Pointer(uintptr(unsafe.Pointer(&g)) + 108 + uintptr(libc.PostIncInt32(&g.nResult, 1)))) = int8(' ')
				}
...
func testset_main(tls *libc.TLS) {
	...
	*(*int8)(unsafe.Pointer(bp + 568)) = int8(0)
	...
	for i = 1; i <= n; i++ {
		if (i-1)%g.nRepeat == 0 {
			...
			*(*int8)(unsafe.Pointer(bp + 568)) = int8('%')
			len = speedtest1_numbername(tls, uint32(i), bp+568+uintptr(1), int32(uint32(unsafe.Sizeof([2000]int8{}))-uint32(2)))
	...
	for i = 1; i <= n; i++ {
		if (i-1)%g.nRepeat == 0 {
			x1 = swizzle(tls, uint32(i), uint32(maxb))
			len = speedtest1_numbername(tls, x1, bp+568, int32(uint32(unsafe.Sizeof([2000]int8{}))-uint32(1)))
		}
...
var zType = *(*[120]int8)(unsafe.Pointer(ts + 21296))
...
func displayLinuxIoStats(tls *libc.TLS, out uintptr) {
	...
	sqlite3.Xsqlite3_snprintf(tls, int32(unsafe.Sizeof([200]int8{})), bp+24, ts+23251, libc.VaList(bp, libc.Xgetpid(tls)))
	...
	for i = 1; i < argc; i++ {
		...
		if int32(*(*int8)(unsafe.Pointer(z))) == '-' {
			...
			if libc.Xstrcmp(tls, z, ts+23594) == 0 {
				...
				i++
				if int32(*(*int8)(unsafe.Pointer(*(*uintptr)(unsafe.Pointer(argv + uintptr(i)*4))))) < '0' || int32(*(*int8)(unsafe.Pointer(*(*uintptr)(unsafe.Pointer(argv + uintptr(i)*4))))) > '9' || int32(*(*int8)(unsafe.Pointer(*(*uintptr)(unsafe.Pointer(argv + uintptr(i)*4)) + 1))) != 0 {
					fatal_error(tls, ts+23998, 0)
				}
				g.eTemp = int32(*(*int8)(unsafe.Pointer(*(*uintptr)(unsafe.Pointer(argv + uintptr(i)*4))))) - '0'
