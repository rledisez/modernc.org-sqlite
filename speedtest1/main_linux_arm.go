...
var zHelp = *(*[2632]uint8)(unsafe.Pointer(ts))
...
func integerValue(tls *libc.TLS, zArg uintptr) int32 {
	...
	for i = 0; uint32(i) < uint32(unsafe.Sizeof(aMult))/uint32(unsafe.Sizeof(struct {
		...
		if sqlite3.Xsqlite3_stricmp(tls, aMult[i].zSuffix, zArg) == 0 {
			v = v * int64(aMult[i].iMult)
			break
		}
	}
	if v > int64(0x7fffffff) {
		fatal_error(tls, ts+2639, 0)
	}
	return func() int32 {
		if isNeg != 0 {
			return int32(-v)
...
var aMult = [9]struct {
	...
	{zSuffix: ts + 2676, iMult: 1024},
	{zSuffix: ts + 2680, iMult: 1024 * 1024},
	{zSuffix: ts + 2684, iMult: 1024 * 1024 * 1024},
	{zSuffix: ts + 2688, iMult: 1000},
	{zSuffix: ts + 2691, iMult: 1000000},
	{zSuffix: ts + 2694, iMult: 1000000000},
	{zSuffix: ts + 2697, iMult: 1000},
	{zSuffix: ts + 2699, iMult: 1000000},
	{zSuffix: ts + 2701, iMult: 1000000000},
}

func speedtest1_timestamp(tls *libc.TLS) int64 {
	bp := tls.Alloc(16)
	defer tls.Free(16)
	...
	if (*sqlite3_vfs)(unsafe.Pointer(clockVfs)).iVersion >= 2 && (*sqlite3_vfs)(unsafe.Pointer(clockVfs)).xCurrentTimeInt64 != uintptr(0) {
		...
		*(*int64)(unsafe.Pointer(bp)) = libc.Int64FromFloat64(*(*float64)(unsafe.Pointer(bp + 8)) * 86400000.0)
	}
...
func speedtest1_numbername(tls *libc.TLS, n uint32, zOut uintptr, nOut int32) int32 {
	...
	if n >= uint32(1000000000) {
		...
		sqlite3.Xsqlite3_snprintf(tls, nOut-i, zOut+uintptr(i), ts+2703, 0)
	...
	if n >= uint32(1000000) {
		...
		sqlite3.Xsqlite3_snprintf(tls, nOut-i, zOut+uintptr(i), ts+2712, 0)
		i = i + int32(libc.Xstrlen(tls, zOut+uintptr(i)))
		n = n % uint32(1000000)
	}
	if n >= uint32(1000) {
		...
		sqlite3.Xsqlite3_snprintf(tls, nOut-i, zOut+uintptr(i), ts+2721, 0)
		i = i + int32(libc.Xstrlen(tls, zOut+uintptr(i)))
		n = n % uint32(1000)
	}
	if n >= uint32(100) {
		...
		sqlite3.Xsqlite3_snprintf(tls, nOut-i, zOut+uintptr(i), ts+2731, libc.VaList(bp, ones[n/uint32(100)]))
		i = i + int32(libc.Xstrlen(tls, zOut+uintptr(i)))
		n = n % uint32(100)
	}
	if n >= uint32(20) {
		...
		sqlite3.Xsqlite3_snprintf(tls, nOut-i, zOut+uintptr(i), ts+2742, libc.VaList(bp+8, tens[n/uint32(10)]))
		i = i + int32(libc.Xstrlen(tls, zOut+uintptr(i)))
		n = n % uint32(10)
	}
	if n > uint32(0) {
		...
		sqlite3.Xsqlite3_snprintf(tls, nOut-i, zOut+uintptr(i), ts+2742, libc.VaList(bp+16, ones[n]))
		i = i + int32(libc.Xstrlen(tls, zOut+uintptr(i)))
	}
	if i == 0 {
		sqlite3.Xsqlite3_snprintf(tls, nOut-i, zOut+uintptr(i), ts+2745, 0)
		i = i + int32(libc.Xstrlen(tls, zOut+uintptr(i)))
	}
	return i
}

var ones = [20]uintptr{ts + 2745, ts + 2750, ts + 2754, ts + 2758, ts + 2764, ts + 2769,
	ts + 2774, ts + 2778, ts + 2784, ts + 2790, ts + 2795, ts + 2799, ts + 2806,
	ts + 2813, ts + 2822, ts + 2831, ts + 2839, ts + 2847,
	ts + 2857, ts + 2866}
var tens = [10]uintptr{ts + 2638, ts + 2795, ts + 2875, ts + 2882, ts + 2889,
	ts + 2895, ts + 2901, ts + 2907, ts + 2915, ts + 2922}

var zDots = *(*[72]uint8)(unsafe.Pointer(ts + 2929))
...
func speedtest1_begin_test(tls *libc.TLS, iTestNum int32, zTestName uintptr, va uintptr) {
	...
	if g.pScript != 0 {
		libc.Xfprintf(tls, g.pScript, ts+3001, libc.VaList(bp, iTestNumber, n, zName))
	}
	if g.bSqlOnly != 0 {
		libc.Xprintf(tls, ts+3024, libc.VaList(bp+24, iTestNum, zName, NAMEWIDTH-n, uintptr(unsafe.Pointer(&zDots))))
	} else {
		libc.Xprintf(tls, ts+3044, libc.VaList(bp+56, iTestNum, zName, NAMEWIDTH-n, uintptr(unsafe.Pointer(&zDots))))
...
func speedtest1_end_test(tls *libc.TLS) {
	...
	var iElapseTime int64 = speedtest1_timestamp(tls) - g.iStart
	if g.doCheckpoint != 0 {
		speedtest1_exec(tls, ts+3058, 0)
	}

	if g.pScript != 0 {
		libc.Xfprintf(tls, g.pScript, ts+3081, libc.VaList(bp, iTestNumber))
	}
	if !(g.bSqlOnly != 0) {
		*(*int64)(unsafe.Pointer(uintptr(unsafe.Pointer(&g)) + 16)) += iElapseTime
		libc.Xprintf(tls, ts+3097, libc.VaList(bp+8, int32(iElapseTime/int64(1000)), int32(iElapseTime%int64(1000))))
	}
	if g.pStmt != 0 {
...
func speedtest1_final(tls *libc.TLS) {
	...
	if !(g.bSqlOnly != 0) {
		libc.Xprintf(tls, ts+3108, libc.VaList(bp, NAMEWIDTH-5, uintptr(unsafe.Pointer(&zDots)),
			int32(g.iTotal/int64(1000)), int32(g.iTotal%int64(1000))))
	}
	if g.bVerify != 0 {
		var i int32
		libc.Xprintf(tls, ts+3136, libc.VaList(bp+32, g.nResByte))
		HashUpdate(tls, ts+3161, uint32(1))
		HashFinal(tls)
		for i = 0; i < 24; i++ {
			libc.Xprintf(tls, ts+3163, libc.VaList(bp+40, int32(*(*uint8)(unsafe.Pointer(uintptr(unsafe.Pointer(&g)) + 3116 + 259 + uintptr(i))))))
		}
		if g.hashFile != 0 && g.hashFile != libc.Xstdout {
			libc.Xfclose(tls, g.hashFile)
		}
		libc.Xprintf(tls, ts+3161, 0)
	}
}
...
func printSql(tls *libc.TLS, zSql uintptr) {
	...
	if g.bExplain != 0 {
		libc.Xprintf(tls, ts+3168, 0)
	}
	libc.Xprintf(tls, ts+3177, libc.VaList(bp, n, zSql))
	if g.bExplain != 0 &&
		(sqlite3.Xsqlite3_strglob(tls, ts+3184, zSql) == 0 ||
			sqlite3.Xsqlite3_strglob(tls, ts+3193, zSql) == 0 ||
			sqlite3.Xsqlite3_strglob(tls, ts+3200, zSql) == 0) {
		libc.Xprintf(tls, ts+3177, libc.VaList(bp+16, n, zSql))
	}
}
...
func speedtest1_exec(tls *libc.TLS, zFormat uintptr, va uintptr) {
	...
	if g.bSqlOnly != 0 {
		...
		*(*uintptr)(unsafe.Pointer(bp + 32)) = uintptr(0)
		var rc int32
		if g.pScript != 0 {
			libc.Xfprintf(tls, g.pScript, ts+3208, libc.VaList(bp, zSql))
		}
		rc = sqlite3.Xsqlite3_exec(tls, g.db, zSql, uintptr(0), uintptr(0), bp+32)
		if *(*uintptr)(unsafe.Pointer(bp + 32)) != 0 {
			fatal_error(tls, ts+3213, libc.VaList(bp+8, *(*uintptr)(unsafe.Pointer(bp + 32)), zSql))
		}
		if rc != SQLITE_OK {
			fatal_error(tls, ts+3231, libc.VaList(bp+24, sqlite3.Xsqlite3_errmsg(tls, g.db)))
		}
	}
...
func speedtest1_once(tls *libc.TLS, zFormat uintptr, va uintptr) uintptr {
	...
	if g.bSqlOnly != 0 {
		...
		if rc != 0 {
			fatal_error(tls, ts+3247, libc.VaList(bp, sqlite3.Xsqlite3_errmsg(tls, g.db)))
		}
		if g.pScript != 0 {
			var z uintptr = sqlite3.Xsqlite3_expanded_sql(tls, *(*uintptr)(unsafe.Pointer(bp + 24)))
			libc.Xfprintf(tls, g.pScript, ts+3262, libc.VaList(bp+8, z))
		...
		if sqlite3.Xsqlite3_step(tls, *(*uintptr)(unsafe.Pointer(bp + 24))) == SQLITE_ROW {
			...
			if z != 0 {
				zResult = sqlite3.Xsqlite3_mprintf(tls, ts+2742, libc.VaList(bp+16, z))
			}
		}
...
func speedtest1_prepare(tls *libc.TLS, zFormat uintptr, va uintptr) {
	...
	if g.bSqlOnly != 0 {
		...
		rc = sqlite3.Xsqlite3_prepare_v2(tls, g.db, zSql, -1, uintptr(unsafe.Pointer(&g))+4, uintptr(0))
		if rc != 0 {
			fatal_error(tls, ts+3247, libc.VaList(bp, sqlite3.Xsqlite3_errmsg(tls, g.db)))
		}
	}
...
func speedtest1_run(tls *libc.TLS) {
	...
	if g.pScript != 0 {
		...
		libc.Xfprintf(tls, g.pScript, ts+3262, libc.VaList(bp, z))
		sqlite3.Xsqlite3_free(tls, z)
	}
	for sqlite3.Xsqlite3_step(tls, g.pStmt) == SQLITE_ROW {
		...
		for i = 0; i < n; i++ {
			...
			if g.bVerify != 0 {
				...
				*(*uint8)(unsafe.Pointer(bp + 8 + 1)) = *(*uint8)(unsafe.Pointer(ts + 3270 + uintptr(eType)))
				if g.nResByte != 0 {
				...
				if eType == SQLITE_FLOAT {
					*(*uint64)(unsafe.Pointer(uintptr(unsafe.Pointer(&g)) + 96)) += uint64(2)
					...
					for iBlob = 0; iBlob < nBlob; iBlob++ {
						*(*uint8)(unsafe.Pointer(bp + 10)) = *(*uint8)(unsafe.Pointer(ts + 3277 + uintptr(int32(*(*uint8)(unsafe.Pointer(aBlob + uintptr(iBlob))))>>4)))
						*(*uint8)(unsafe.Pointer(bp + 10 + 1)) = *(*uint8)(unsafe.Pointer(ts + 3277 + uintptr(int32(*(*uint8)(unsafe.Pointer(aBlob + uintptr(iBlob))))&15)))
						HashUpdate(tls, bp+10, uint32(2))
					}
					*(*uint64)(unsafe.Pointer(uintptr(unsafe.Pointer(&g)) + 96)) += uint64(nBlob*2 + 2)
				} else {
					HashUpdate(tls, z, uint32(len))
					*(*uint64)(unsafe.Pointer(uintptr(unsafe.Pointer(&g)) + 96)) += uint64(len + 2)
				}
			}
			if uint32(g.nResult+len) < uint32(unsafe.Sizeof([3000]uint8{}))-uint32(2) {
				if g.nResult > 0 {
					*(*uint8)(unsafe.Pointer(uintptr(unsafe.Pointer(&g)) + 108 + uintptr(libc.PostIncInt32(&g.nResult, 1)))) = uint8(' ')
				}
				libc.Xmemcpy(tls, uintptr(unsafe.Pointer(&g))+108+uintptr(g.nResult), z, uint32(len+1))
				*(*int32)(unsafe.Pointer(uintptr(unsafe.Pointer(&g)) + 104)) += len
			}
		}
	}
	if g.bReprepare != 0 {
		sqlite3.Xsqlite3_prepare_v2(tls, g.db, sqlite3.Xsqlite3_sql(tls, g.pStmt), -1, bp+12, uintptr(0))
		sqlite3.Xsqlite3_finalize(tls, g.pStmt)
		g.pStmt = *(*uintptr)(unsafe.Pointer(bp + 12))
	} else {
...
func traceCallback(tls *libc.TLS, NotUsed uintptr, zSql uintptr) {
	...
	libc.Xfprintf(tls, libc.Xstderr, ts+3177, libc.VaList(bp, n, zSql))
}

func randomFunc(tls *libc.TLS, context uintptr, NotUsed int32, NotUsed2 uintptr) {
	sqlite3.Xsqlite3_result_int64(tls, context, int64(speedtest1_random(tls)))
}
...
func testset_main(tls *libc.TLS) {
	...
	*(*uint8)(unsafe.Pointer(bp + 568)) = uint8(0)
	...
	speedtest1_begin_test(tls, 100, ts+3294, libc.VaList(bp, n))
	speedtest1_exec(tls, ts+3330, 0)
	speedtest1_exec(tls, ts+3336,
		libc.VaList(bp+8, isTemp(tls, 9), g.zNN, g.zNN, g.zNN))
	speedtest1_prepare(tls, ts+3394, libc.VaList(bp+40, n))
	for i = 1; i <= n; i++ {
		x1 = swizzle(tls, uint32(i), uint32(maxb))
		speedtest1_numbername(tls, x1, bp+568, int32(unsafe.Sizeof([2000]uint8{})))
		sqlite3.Xsqlite3_bind_int64(tls, g.pStmt, 1, int64(x1))
		sqlite3.Xsqlite3_bind_int(tls, g.pStmt, 2, i)
		sqlite3.Xsqlite3_bind_text(tls, g.pStmt, 3, bp+568, -1, uintptr(0))
		speedtest1_run(tls)
	}
	speedtest1_exec(tls, ts+3440, 0)
	speedtest1_end_test(tls)

	n = sz
	speedtest1_begin_test(tls, 110, ts+3447, libc.VaList(bp+48, n))
	speedtest1_exec(tls, ts+3330, 0)
	speedtest1_exec(tls,
		ts+3484,
		libc.VaList(bp+56, isTemp(tls, 5), g.zNN, g.zPK, g.zNN, g.zNN, g.zWR))
	speedtest1_prepare(tls, ts+3547, libc.VaList(bp+104, n))
	for i = 1; i <= n; i++ {
		x1 = swizzle(tls, uint32(i), uint32(maxb))
		speedtest1_numbername(tls, x1, bp+568, int32(unsafe.Sizeof([2000]uint8{})))
		sqlite3.Xsqlite3_bind_int(tls, g.pStmt, 1, i)
		sqlite3.Xsqlite3_bind_int64(tls, g.pStmt, 2, int64(x1))
		sqlite3.Xsqlite3_bind_text(tls, g.pStmt, 3, bp+568, -1, uintptr(0))
		speedtest1_run(tls)
	}
	speedtest1_exec(tls, ts+3440, 0)
	...
	n = sz
	speedtest1_begin_test(tls, 120, ts+3592, libc.VaList(bp+112, n))
	speedtest1_exec(tls, ts+3330, 0)
	speedtest1_exec(tls,
		ts+3631,
		libc.VaList(bp+120, isTemp(tls, 3), g.zNN, g.zPK, g.zNN, g.zNN, g.zWR))
	speedtest1_prepare(tls, ts+3694, libc.VaList(bp+168, n))
	for i = 1; i <= n; i++ {
		x1 = swizzle(tls, uint32(i), uint32(maxb))
		speedtest1_numbername(tls, x1, bp+568, int32(unsafe.Sizeof([2000]uint8{})))
		sqlite3.Xsqlite3_bind_int(tls, g.pStmt, 2, i)
	...
	n = 25
	speedtest1_begin_test(tls, 130, ts+3739, libc.VaList(bp+176, n))
	speedtest1_exec(tls, ts+3330, 0)
	speedtest1_prepare(tls,
		ts+3778, libc.VaList(bp+184, n))
	for i = 1; i <= n; i++ {
		if (i-1)%g.nRepeat == 0 {
	...
	n = 10
	speedtest1_begin_test(tls, 140, ts+3883, libc.VaList(bp+192, n))
	speedtest1_exec(tls, ts+3330, 0)
	speedtest1_prepare(tls,
		ts+3911, libc.VaList(bp+200, n))
	for i = 1; i <= n; i++ {
		if (i-1)%g.nRepeat == 0 {
			x1 = speedtest1_random(tls) % uint32(maxb)
			*(*uint8)(unsafe.Pointer(bp + 568)) = uint8('%')
			len = speedtest1_numbername(tls, uint32(i), bp+568+uintptr(1), int32(uint32(unsafe.Sizeof([2000]uint8{}))-uint32(2)))
			*(*uint8)(unsafe.Pointer(bp + 568 + uintptr(len))) = uint8('%')
			*(*uint8)(unsafe.Pointer(bp + 568 + uintptr(len+1))) = uint8(0)
		}
	...
	n = 10
	speedtest1_begin_test(tls, 142, ts+4006, libc.VaList(bp+208, n))
	speedtest1_exec(tls, ts+3330, 0)
	speedtest1_prepare(tls,
		ts+4039, libc.VaList(bp+216, n))
	for i = 1; i <= n; i++ {
		if (i-1)%g.nRepeat == 0 {
			x1 = speedtest1_random(tls) % uint32(maxb)
			*(*uint8)(unsafe.Pointer(bp + 568)) = uint8('%')
	...
	n = 10
	speedtest1_begin_test(tls, 145, ts+4103, libc.VaList(bp+224, n))
	speedtest1_exec(tls, ts+3330, 0)
	speedtest1_prepare(tls,
		ts+4146, libc.VaList(bp+232, n))
	for i = 1; i <= n; i++ {
		if (i-1)%g.nRepeat == 0 {
			x1 = speedtest1_random(tls) % uint32(maxb)
			*(*uint8)(unsafe.Pointer(bp + 568)) = uint8('%')
	...
	speedtest1_begin_test(tls, 150, ts+4219, 0)
	speedtest1_exec(tls, ts+4243, 0)
	speedtest1_exec(tls, ts+4250, 0)
	speedtest1_exec(tls, ts+4284, 0)
	speedtest1_exec(tls, ts+4311, 0)
	speedtest1_exec(tls, ts+4345, 0)
	speedtest1_exec(tls, ts+4377, 0)
	speedtest1_exec(tls, ts+4407, 0)
	speedtest1_end_test(tls)

	n = sz / 5
	speedtest1_begin_test(tls, 160, ts+4415, libc.VaList(bp+240, n))
	speedtest1_exec(tls, ts+3330, 0)
	speedtest1_prepare(tls,
		ts+4452, libc.VaList(bp+248, n))
	for i = 1; i <= n; i++ {
		if (i-1)%g.nRepeat == 0 {
			x1 = speedtest1_random(tls) % uint32(maxb)
			x2 = speedtest1_random(tls)%uint32(10) + uint32(sz/5000) + x1
	...
	n = sz / 5
	speedtest1_begin_test(tls, 161, ts+4557, libc.VaList(bp+256, n))
	speedtest1_exec(tls, ts+3330, 0)
	speedtest1_prepare(tls,
		ts+4589, libc.VaList(bp+264, n))
	for i = 1; i <= n; i++ {
		if (i-1)%g.nRepeat == 0 {
			x1 = speedtest1_random(tls) % uint32(maxb)
			x2 = speedtest1_random(tls)%uint32(10) + uint32(sz/5000) + x1
	...
	n = sz / 5
	speedtest1_begin_test(tls, 170, ts+4694, libc.VaList(bp+272, n))
	speedtest1_exec(tls, ts+3330, 0)
	speedtest1_prepare(tls,
		ts+4728, libc.VaList(bp+280, n))
	for i = 1; i <= n; i++ {
		if (i-1)%g.nRepeat == 0 {
			x1 = swizzle(tls, uint32(i), uint32(maxb))
			len = speedtest1_numbername(tls, x1, bp+568, int32(uint32(unsafe.Sizeof([2000]uint8{}))-uint32(1)))
		}
	...
	n = sz
	speedtest1_begin_test(tls, 180, ts+4840, libc.VaList(bp+288, n))
	speedtest1_exec(tls, ts+3330, 0)
	speedtest1_exec(tls,
		ts+4870,
		libc.VaList(bp+296, isTemp(tls, 1), g.zNN, g.zPK, g.zNN, g.zNN, g.zWR))
	speedtest1_exec(tls, ts+4941, 0)
	speedtest1_exec(tls, ts+4967, 0)
	speedtest1_exec(tls, ts+4993, 0)
	speedtest1_exec(tls, ts+3440, 0)
	speedtest1_end_test(tls)

	n = sz
	speedtest1_begin_test(tls, 190, ts+5025, libc.VaList(bp+344, n))
	speedtest1_exec(tls, ts+5053, 0)
	speedtest1_exec(tls, ts+5069, 0)
	speedtest1_end_test(tls)

	speedtest1_begin_test(tls, 200, ts+5102, 0)
	speedtest1_exec(tls, ts+5102, 0)
	speedtest1_end_test(tls)

	speedtest1_begin_test(tls, 210, ts+5109, 0)
	speedtest1_exec(tls, ts+5143, 0)
	speedtest1_exec(tls, ts+5187, 0)
	speedtest1_end_test(tls)

	n = sz / 5
	speedtest1_begin_test(tls, 230, ts+5209, libc.VaList(bp+352, n))
	speedtest1_exec(tls, ts+3330, 0)
	speedtest1_prepare(tls,
		ts+5246, libc.VaList(bp+360, n))
	for i = 1; i <= n; i++ {
		x1 = speedtest1_random(tls) % uint32(maxb)
		x2 = speedtest1_random(tls)%uint32(10) + uint32(sz/5000) + x1
		sqlite3.Xsqlite3_bind_int(tls, g.pStmt, 1, int32(x1))
	...
	n = sz
	speedtest1_begin_test(tls, 240, ts+5305, libc.VaList(bp+368, n))
	speedtest1_exec(tls, ts+3330, 0)
	speedtest1_prepare(tls,
		ts+5335, libc.VaList(bp+376, n))
	for i = 1; i <= n; i++ {
	...
	speedtest1_begin_test(tls, 250, ts+5379, libc.VaList(bp+384, sz))
	speedtest1_exec(tls, ts+5420, 0)
	speedtest1_end_test(tls)

	speedtest1_begin_test(tls, 260, ts+5440, 0)
	speedtest1_exec(tls, ts+5187, 0)
	speedtest1_end_test(tls)

	n = sz / 5
	speedtest1_begin_test(tls, 270, ts+5473, libc.VaList(bp+392, n))
	speedtest1_exec(tls, ts+3330, 0)
	speedtest1_prepare(tls,
		ts+5510, libc.VaList(bp+400, n))
	for i = 1; i <= n; i++ {
	...
	n = sz
	speedtest1_begin_test(tls, 280, ts+5564, libc.VaList(bp+408, n))
	speedtest1_exec(tls, ts+3330, 0)
	speedtest1_prepare(tls,
		ts+5594, libc.VaList(bp+416, n))
	for i = 1; i <= n; i++ {
		x1 = speedtest1_random(tls)%uint32(sz) + uint32(1)
		sqlite3.Xsqlite3_bind_int(tls, g.pStmt, 1, int32(x1))
		speedtest1_run(tls)
	...
	speedtest1_begin_test(tls, 290, ts+5633, libc.VaList(bp+424, sz))
	speedtest1_exec(tls, ts+5672, 0)
	speedtest1_exec(tls, ts+5716, 0)
	speedtest1_end_test(tls)

	speedtest1_begin_test(tls, 300, ts+5760, libc.VaList(bp+432, sz))
	speedtest1_exec(tls, ts+5053, 0)
	speedtest1_exec(tls,
		ts+5801, 0)
	speedtest1_exec(tls,
	...
	n = sz / 5
	speedtest1_begin_test(tls, 310, ts+5931, libc.VaList(bp+440, n))
	speedtest1_exec(tls, ts+3330, 0)
	speedtest1_prepare(tls,
		ts+5950, 0)
	for i = 1; i <= n; i++ {
	...
	speedtest1_begin_test(tls, 320, ts+6064, libc.VaList(bp+448, n))
	speedtest1_prepare(tls,
		ts+6087, 0)
	...
	*(*uint8)(unsafe.Pointer(bp + 568)) = uint8(0)
	...
	speedtest1_begin_test(tls, 400, ts+6205, libc.VaList(bp+456, n))
	speedtest1_exec(tls, ts+3330, 0)
	speedtest1_exec(tls, ts+6230,
		libc.VaList(bp+464, isTemp(tls, 9), g.zNN))
	speedtest1_prepare(tls, ts+6278, libc.VaList(bp+480, n))
	for i = 1; i <= n; i++ {
		x1 = swizzle(tls, uint32(i), uint32(maxb))
		speedtest1_numbername(tls, uint32(i), bp+568, int32(unsafe.Sizeof([2000]uint8{})))
		sqlite3.Xsqlite3_bind_int(tls, g.pStmt, 1, int32(int64(x1)))
		sqlite3.Xsqlite3_bind_text(tls, g.pStmt, 2, bp+568, -1, uintptr(0))
		speedtest1_run(tls)
	}
	speedtest1_exec(tls, ts+3440, 0)
	speedtest1_end_test(tls)
	speedtest1_begin_test(tls, 410, ts+6322, libc.VaList(bp+488, n))
	if g.doBigTransactions != 0 {
		speedtest1_exec(tls, ts+3330, 0)
	}
	speedtest1_prepare(tls, ts+6343, libc.VaList(bp+496, n))
	for i = 1; i <= n; i++ {
		x1 = swizzle(tls, uint32(i), uint32(maxb))
		sqlite3.Xsqlite3_bind_int(tls, g.pStmt, 1, int32(int64(x1)))
		speedtest1_run(tls)
	...
	*(*uint8)(unsafe.Pointer(bp + 568)) = uint8(0)
	maxb = int32(roundup_allones(tls, uint32(sz/3)))
	speedtest1_begin_test(tls, 500, ts+6385, libc.VaList(bp+504, n))
	speedtest1_exec(tls, ts+3330, 0)
	speedtest1_exec(tls, ts+6407,
	...
	speedtest1_prepare(tls, ts+6468, libc.VaList(bp+536, n))
	for i = 1; i <= n; i++ {
		x1 = swizzle(tls, uint32(i), uint32(maxb))
		speedtest1_numbername(tls, x1, bp+568, int32(unsafe.Sizeof([2000]uint8{})))
		sqlite3.Xsqlite3_bind_int(tls, g.pStmt, 2, i)
	...
	speedtest1_exec(tls, ts+3440, 0)
	speedtest1_end_test(tls)
	speedtest1_begin_test(tls, 510, ts+6512, libc.VaList(bp+544, n))
	if g.doBigTransactions != 0 {
		speedtest1_exec(tls, ts+3330, 0)
	}
	speedtest1_prepare(tls, ts+6536, libc.VaList(bp+552, n))
	for i = 1; i <= n; i++ {
		x1 = swizzle(tls, uint32(i), uint32(maxb))
		speedtest1_numbername(tls, x1, bp+568, int32(unsafe.Sizeof([2000]uint8{})))
		sqlite3.Xsqlite3_bind_text(tls, g.pStmt, 1, bp+568, -1, uintptr(0))
	...
	speedtest1_end_test(tls)
	speedtest1_begin_test(tls, 520, ts+6578, libc.VaList(bp+560, n))
	speedtest1_exec(tls, ts+6597, 0)
	speedtest1_exec(tls, ts+6624, 0)
	speedtest1_end_test(tls)

	speedtest1_begin_test(tls, 980, ts+6651, 0)
	speedtest1_exec(tls, ts+6651, 0)
	speedtest1_end_test(tls)

	speedtest1_begin_test(tls, 990, ts+6674, 0)
	speedtest1_exec(tls, ts+6674, 0)
	speedtest1_end_test(tls)
}
...
func testset_cte(tls *libc.TLS) {
	...
	speedtest1_begin_test(tls, 100, ts+6682, 0)
	speedtest1_prepare(tls,
		ts+6713, 0)
	...
	speedtest1_begin_test(tls, 200, ts+7517, 0)
	speedtest1_prepare(tls,
		ts+7545, 0)
	sqlite3.Xsqlite3_bind_text(tls, g.pStmt, 1, zPuz, -1, uintptr(0))
	speedtest1_run(tls)
	...
	speedtest1_begin_test(tls, 300, ts+8357, libc.VaList(bp, rSpacing))
	speedtest1_prepare(tls,
		ts+8388, 0)
	...
	speedtest1_begin_test(tls, 400, ts+8984, libc.VaList(bp+8, nElem))
	speedtest1_prepare(tls,
		ts+9021,
...
func speedtest1_random_ascii_fp(tls *libc.TLS, zFP uintptr) {
	...
	y = y / 10
	sqlite3.Xsqlite3_snprintf(tls, 100, zFP, ts+9498, libc.VaList(bp, y, z, x%200))
}
...
func testset_fp(tls *libc.TLS) {
	...
	n = g.szTest * 5000
	speedtest1_begin_test(tls, 100, ts+9507, libc.VaList(bp, n*2))
	speedtest1_exec(tls, ts+3330, 0)
	speedtest1_exec(tls, ts+9538,
		libc.VaList(bp+8, isTemp(tls, 1), g.zNN, g.zNN))
	speedtest1_prepare(tls, ts+9579, libc.VaList(bp+32, n))
	for i = 1; i <= n; i++ {
	...
	n = g.szTest/25 + 2
	speedtest1_begin_test(tls, 110, ts+9621, libc.VaList(bp+40, n))
	speedtest1_prepare(tls, ts+9638, 0)
	for i = 1; i <= n; i++ {
		speedtest1_random_ascii_fp(tls, bp+72)
		speedtest1_random_ascii_fp(tls, bp+172)
		sqlite3.Xsqlite3_bind_text(tls, g.pStmt, 1, bp+72, -1, uintptr(0))
	...
	speedtest1_begin_test(tls, 120, ts+9686, 0)
	speedtest1_exec(tls, ts+4243, 0)
	speedtest1_exec(tls, ts+9711, 0)
	speedtest1_exec(tls, ts+9738, 0)
	speedtest1_exec(tls, ts+9765, 0)
	speedtest1_exec(tls, ts+4407, 0)
	speedtest1_end_test(tls)

	n = g.szTest/3 + 2
	speedtest1_begin_test(tls, 130, ts+9795, libc.VaList(bp+48, n))
	speedtest1_prepare(tls, ts+9638, 0)
	for i = 1; i <= n; i++ {
		speedtest1_random_ascii_fp(tls, bp+72)
		speedtest1_random_ascii_fp(tls, bp+172)
	...
	n = g.szTest * 5000
	speedtest1_begin_test(tls, 140, ts+9820, libc.VaList(bp+56, n))
	speedtest1_exec(tls, ts+9840, 0)
	speedtest1_end_test(tls)

	speedtest1_begin_test(tls, 150, ts+9883, libc.VaList(bp+64, n*4))
	speedtest1_exec(tls,
		ts+9901, 0)
	speedtest1_end_test(tls)
}
...
func xsliceGeometryCallback(tls *libc.TLS, p uintptr, nCoord int32, aCoord uintptr, pRes uintptr) int32 {
	*(*int32)(unsafe.Pointer(pRes)) = libc.Bool32(*(*float64)(unsafe.Pointer(aCoord + 3*8)) >= *(*float64)(unsafe.Pointer((*sqlite3_rtree_geometry)(unsafe.Pointer(p)).aParam)) && *(*float64)(unsafe.Pointer(aCoord + 2*8)) <= *(*float64)(unsafe.Pointer((*sqlite3_rtree_geometry)(unsafe.Pointer(p)).aParam + 1*8)))
	return SQLITE_OK
}
...
func testset_rtree(tls *libc.TLS, p1 int32, p2 int32) {
	...
	speedtest1_begin_test(tls, 100, ts+9996, libc.VaList(bp, n))
	speedtest1_exec(tls, ts+3330, 0)
	speedtest1_exec(tls, ts+10022, 0)
	speedtest1_prepare(tls,
		ts+10081, 0)
	...
	speedtest1_begin_test(tls, 101, ts+10147, 0)
	speedtest1_exec(tls, ts+10182, 0)
	speedtest1_exec(tls, ts+10240, 0)
	speedtest1_end_test(tls)

	n = uint32(g.szTest * 200)
	speedtest1_begin_test(tls, 110, ts+10273, libc.VaList(bp+8, n))
	speedtest1_prepare(tls, ts+10316, 0)
	iStep = mxCoord / n
	for i = uint32(0); i < n; i++ {
		sqlite3.Xsqlite3_bind_int(tls, g.pStmt, 1, int32(i*iStep))
		sqlite3.Xsqlite3_bind_int(tls, g.pStmt, 2, int32((i+uint32(1))*iStep))
		speedtest1_run(tls)
		*(*int32)(unsafe.Pointer(aCheck + uintptr(i)*4)) = libc.Xatoi(tls, uintptr(unsafe.Pointer(&g))+108)
	}
	speedtest1_end_test(tls)

	if g.bVerify != 0 {
		n = uint32(g.szTest * 200)
		speedtest1_begin_test(tls, 111, ts+10365, 0)
		speedtest1_prepare(tls, ts+10412, 0)
		iStep = mxCoord / n
		for i = uint32(0); i < n; i++ {
			sqlite3.Xsqlite3_bind_int(tls, g.pStmt, 1, int32(i*iStep))
			sqlite3.Xsqlite3_bind_int(tls, g.pStmt, 2, int32((i+uint32(1))*iStep))
			speedtest1_run(tls)
			if *(*int32)(unsafe.Pointer(aCheck + uintptr(i)*4)) != libc.Xatoi(tls, uintptr(unsafe.Pointer(&g))+108) {
				fatal_error(tls, ts+10460,
					libc.VaList(bp+16, i, i*iStep, (i+uint32(1))*iStep, *(*int32)(unsafe.Pointer(aCheck + uintptr(i)*4)), libc.Xatoi(tls, uintptr(unsafe.Pointer(&g))+108)))
			}
		}
		speedtest1_end_test(tls)
	}
	...
	n = uint32(g.szTest * 200)
	speedtest1_begin_test(tls, 120, ts+10502, libc.VaList(bp+56, n))
	speedtest1_prepare(tls, ts+10543, 0)
	iStep = mxCoord / n
	for i = uint32(0); i < n; i++ {
		sqlite3.Xsqlite3_bind_int(tls, g.pStmt, 1, int32(i*iStep))
		sqlite3.Xsqlite3_bind_int(tls, g.pStmt, 2, int32((i+uint32(1))*iStep))
	...
	if g.bVerify != 0 {
		n = uint32(g.szTest * 200)
		speedtest1_begin_test(tls, 121, ts+10592, 0)
		speedtest1_prepare(tls, ts+10637, 0)
		iStep = mxCoord / n
		for i = uint32(0); i < n; i++ {
			sqlite3.Xsqlite3_bind_int(tls, g.pStmt, 1, int32(i*iStep))
			sqlite3.Xsqlite3_bind_int(tls, g.pStmt, 2, int32((i+uint32(1))*iStep))
			...
			if *(*int32)(unsafe.Pointer(aCheck + uintptr(i)*4)) != libc.Xatoi(tls, uintptr(unsafe.Pointer(&g))+108) {
				fatal_error(tls, ts+10460,
					libc.VaList(bp+64, i, i*iStep, (i+uint32(1))*iStep, *(*int32)(unsafe.Pointer(aCheck + uintptr(i)*4)), libc.Xatoi(tls, uintptr(unsafe.Pointer(&g))+108)))
			}
		}
		speedtest1_end_test(tls)
	}
	...
	n = uint32(g.szTest * 200)
	speedtest1_begin_test(tls, 125, ts+10685, libc.VaList(bp+104, n))
	sqlite3.Xsqlite3_rtree_geometry_callback(tls, g.db, ts+10721, *(*uintptr)(unsafe.Pointer(&struct {
		f func(*libc.TLS, uintptr, int32, uintptr, uintptr) int32
	}{xsliceGeometryCallback})), uintptr(0))
	speedtest1_prepare(tls, ts+10728, 0)
	iStep = mxCoord / n
	for i = uint32(0); i < n; i++ {
		sqlite3.Xsqlite3_bind_int(tls, g.pStmt, 1, int32(i*iStep))
		sqlite3.Xsqlite3_bind_int(tls, g.pStmt, 2, int32((i+uint32(1))*iStep))
		...
		if *(*int32)(unsafe.Pointer(aCheck + uintptr(i)*4)) != libc.Xatoi(tls, uintptr(unsafe.Pointer(&g))+108) {
			fatal_error(tls, ts+10460,
				libc.VaList(bp+112, i, i*iStep, (i+uint32(1))*iStep, *(*int32)(unsafe.Pointer(aCheck + uintptr(i)*4)), libc.Xatoi(tls, uintptr(unsafe.Pointer(&g))+108)))
		}
	}
	speedtest1_end_test(tls)

	n = uint32(g.szTest * 400)
	speedtest1_begin_test(tls, 130, ts+10782, libc.VaList(bp+152, n))
	speedtest1_prepare(tls,
		ts+10825, 0)
	iStep = mxCoord / n
	for i = uint32(0); i < n; i++ {
	...
	speedtest1_begin_test(tls, 140, ts+10918, libc.VaList(bp+160, n))
	speedtest1_prepare(tls, ts+10935, 0)
	for i = uint32(1); i <= n; i++ {
		sqlite3.Xsqlite3_bind_int(tls, g.pStmt, 1, int32(i))
		speedtest1_run(tls)
	}
	...
	speedtest1_begin_test(tls, 150, ts+10965, libc.VaList(bp+168, n))
	speedtest1_prepare(tls, ts+10988, 0)
	for i = uint32(1); i <= n; i++ {
	...
	n = uint32(g.szTest * 5)
	speedtest1_begin_test(tls, 155, ts+11036, libc.VaList(bp+176, n))
	speedtest1_prepare(tls,
		ts+11077, 0)
	iStep = mxCoord / n
	for i = uint32(0); i < n; i++ {
	...
	n = uint32(g.szTest * 50)
	speedtest1_begin_test(tls, 160, ts+11139, libc.VaList(bp+184, n))
	speedtest1_prepare(tls, ts+11162, 0)
	for i = uint32(1); i <= n; i++ {
	...
	n = uint32(g.szTest * 5)
	speedtest1_begin_test(tls, 165, ts+11190, libc.VaList(bp+192, n))
	speedtest1_prepare(tls, ts+11231, 0)
	iStep = mxCoord / n
	for i = uint32(0); i < n; i++ {
		sqlite3.Xsqlite3_bind_int(tls, g.pStmt, 1, int32(i*iStep))
		speedtest1_run(tls)
	...
	speedtest1_begin_test(tls, 170, ts+11273, 0)
	speedtest1_exec(tls, ts+11320, 0)
	speedtest1_end_test(tls)
}
...
func testset_orm(tls *libc.TLS) {
	...
	speedtest1_begin_test(tls, 100, ts+11363, libc.VaList(bp, n))
	speedtest1_exec(tls,
		ts+11376, 0)
	speedtest1_prepare(tls,
		ts+15276, 0)
	for i = uint32(0); i < n; i++ {
		...
		speedtest1_numbername(tls, x1%uint32(1000), bp+16, int32(unsafe.Sizeof([2000]uint8{})))
	...
	speedtest1_begin_test(tls, 110, ts+18520, libc.VaList(bp+8, n))
	speedtest1_prepare(tls,
		ts+18543, 0)
	for i = uint32(0); i < n; i++ {
...
var zType = *(*[120]uint8)(unsafe.Pointer(ts + 21296))
...
func testset_trigger(tls *libc.TLS) {
	...
	speedtest1_exec(tls,
		ts+21416, 0)
	for jj = 1; jj <= 3; jj++ {
		speedtest1_prepare(tls, ts+21744, libc.VaList(bp, jj))
		for ii = 0; ii < NROW; ii++ {
			var x1 int32 = int32(speedtest1_random(tls) % uint32(NROW))
			speedtest1_numbername(tls, uint32(x1), bp+32, int32(unsafe.Sizeof([2000]uint8{})))
	...
	speedtest1_begin_test(tls, 100, ts+21862, 0)
	speedtest1_prepare(tls,
		ts+21876, 0)
	speedtest1_run(tls)
	speedtest1_end_test(tls)

	speedtest1_begin_test(tls, 110, ts+21943, 0)
	speedtest1_prepare(tls,
		ts+21957, 0)
	speedtest1_run(tls)
	speedtest1_end_test(tls)

	speedtest1_begin_test(tls, 120, ts+22016, 0)
	for jj = 1; jj <= 3; jj++ {
		speedtest1_prepare(tls, ts+22030, libc.VaList(bp+8, jj))
	...
	speedtest1_begin_test(tls, 130, ts+22064, 0)
	for jj = 1; jj <= 3; jj++ {
		speedtest1_prepare(tls, ts+22079, libc.VaList(bp+16, jj))
		for ii = 0; ii < NROW2; ii = ii + 3 {
			sqlite3.Xsqlite3_bind_int(tls, g.pStmt, 1, ii*3)
			speedtest1_run(tls)
		}
	...
	speedtest1_begin_test(tls, 140, ts+22064, 0)
	for jj = 1; jj <= 3; jj++ {
		speedtest1_prepare(tls, ts+22079, libc.VaList(bp+24, jj))
		for ii = 0; ii < NROW2; ii = ii + 3 {
			sqlite3.Xsqlite3_bind_int(tls, g.pStmt, 1, ii*3)
			speedtest1_run(tls)
		}
	...
	speedtest1_begin_test(tls, 150, ts+22113, 0)
	speedtest1_prepare(tls,
		ts+22132, 0)
	...
	speedtest1_begin_test(tls, 160, ts+22247, 0)
	speedtest1_exec(tls, ts+3330, 0)
	speedtest1_prepare(tls, ts+22268, 0)
	for jj = 0; jj < NROW2; jj++ {
	...
	speedtest1_exec(tls, ts+22303, 0)
	speedtest1_begin_test(tls, 170, ts+22351, 0)
	speedtest1_exec(tls, ts+22373, 0)
	speedtest1_end_test(tls)

	speedtest1_exec(tls,
		ts+22418, 0)

	speedtest1_begin_test(tls, 180, ts+22908, 0)
	speedtest1_prepare(tls, ts+22925, 0)
	for jj = 0; jj < NROW2; jj++ {
		speedtest1_numbername(tls, uint32(jj), bp+32, int32(unsafe.Sizeof([2000]uint8{})))
		sqlite3.Xsqlite3_bind_int(tls, g.pStmt, 1, jj)
		sqlite3.Xsqlite3_bind_text(tls, g.pStmt, 2, bp+32, -1, uintptr(0))
		speedtest1_run(tls)
	}
	...
	speedtest1_begin_test(tls, 190, ts+22961, 0)
	speedtest1_prepare(tls, ts+22978, 0)
	for jj = 1; jj <= NROW2*2; jj = jj + 2 {
		speedtest1_numbername(tls, uint32(jj*2), bp+32, int32(unsafe.Sizeof([2000]uint8{})))
	...
	speedtest1_begin_test(tls, 200, ts+23024, 0)
	speedtest1_prepare(tls, ts+23041, 0)
	for jj = 1; jj <= NROW2*2; jj = jj + 2 {
		sqlite3.Xsqlite3_bind_int(tls, g.pStmt, 1, jj*2)
		speedtest1_run(tls)
	}
	...
	speedtest1_exec(tls,
		ts+23073, 0)
	speedtest1_begin_test(tls, 210, ts+23178, 0)
	speedtest1_prepare(tls, ts+22925, 0)
	for jj = 0; jj < NROW2; jj++ {
		speedtest1_numbername(tls, uint32(jj), bp+32, int32(unsafe.Sizeof([2000]uint8{})))
		sqlite3.Xsqlite3_bind_int(tls, g.pStmt, 1, jj)
	...
	speedtest1_end_test(tls)
	speedtest1_begin_test(tls, 210, ts+23197, 0)
	speedtest1_prepare(tls, ts+22978, 0)
	for jj = 1; jj <= NROW2*2; jj = jj + 2 {
		speedtest1_numbername(tls, uint32(jj*2), bp+32, int32(unsafe.Sizeof([2000]uint8{})))
		sqlite3.Xsqlite3_bind_int(tls, g.pStmt, 1, jj*2)
	...
	speedtest1_end_test(tls)
	speedtest1_begin_test(tls, 220, ts+23216, 0)
	speedtest1_prepare(tls, ts+23041, 0)
	for jj = 1; jj <= NROW2*2; jj = jj + 2 {
		sqlite3.Xsqlite3_bind_int(tls, g.pStmt, 1, jj*2)
		speedtest1_run(tls)
...
func testset_debug1(tls *libc.TLS) {
	...
	for i = uint32(1); i <= n; i++ {
		x1 = swizzle(tls, i, n)
		x2 = swizzle(tls, x1, n)
		speedtest1_numbername(tls, x1, bp+32, int32(unsafe.Sizeof([2000]uint8{})))
		libc.Xprintf(tls, ts+23235, libc.VaList(bp, i, x1, x2, bp+32))
	}
}
...
func displayLinuxIoStats(tls *libc.TLS, out uintptr) {
	...
	sqlite3.Xsqlite3_snprintf(tls, int32(unsafe.Sizeof([200]uint8{})), bp+24, ts+23251, libc.VaList(bp, libc.Xgetpid(tls)))
	in = libc.Xfopen(tls, bp+24, ts+23263)
	if in == uintptr(0) {
		return
	}
	for libc.Xfgets(tls, bp+24, int32(unsafe.Sizeof([200]uint8{})), in) != uintptr(0) {
		var i int32
		for i = 0; uint32(i) < uint32(unsafe.Sizeof(aTrans))/uint32(unsafe.Sizeof(struct {
			zPattern uintptr
			zDesc    uintptr
		}{})); i++ {
			var n int32 = int32(libc.Xstrlen(tls, aTrans[i].zPattern))
			if libc.Xstrncmp(tls, aTrans[i].zPattern, bp+24, uint32(n)) == 0 {
				libc.Xfprintf(tls, out, ts+23266, libc.VaList(bp+8, aTrans[i].zDesc, bp+24+uintptr(n)))
				break
			}
		}
	}
...
var aTrans = [7]struct {
	...
	{zPattern: ts + 23278, zDesc: ts + 23286},
	{zPattern: ts + 23312, zDesc: ts + 23320},
	{zPattern: ts + 23343, zDesc: ts + 23351},
	{zPattern: ts + 23372, zDesc: ts + 23380},
	{zPattern: ts + 23402, zDesc: ts + 23415},
	{zPattern: ts + 23440, zDesc: ts + 23454},
	{zPattern: ts + 23477, zDesc: ts + 23501},
}
...
func xCompileOptions(tls *libc.TLS, pCtx uintptr, nVal int32, azVal uintptr, azCol uintptr) int32 {
	...
	libc.Xprintf(tls, ts+23524, libc.VaList(bp, *(*uintptr)(unsafe.Pointer(azVal))))
	return SQLITE_OK
}
...
func main1(tls *libc.TLS, argc int32, argv uintptr) int32 {
	...
	for i = 1; i < argc; i++ {
		...
		if int32(*(*uint8)(unsafe.Pointer(z))) == '-' {
			for __ccgo := true; __ccgo; __ccgo = int32(*(*uint8)(unsafe.Pointer(z))) == '-' {
				z++
			}
			if libc.Xstrcmp(tls, z, ts+23594) == 0 {
				doAutovac = 1
			} else if libc.Xstrcmp(tls, z, ts+23605) == 0 {
				g.doBigTransactions = 1
			} else if libc.Xstrcmp(tls, z, ts+23622) == 0 {
				if i >= argc-1 {
					fatal_error(tls, ts+23632, libc.VaList(bp+16, *(*uintptr)(unsafe.Pointer(argv + uintptr(i)*4))))
				}
				cacheSize = integerValue(tls, *(*uintptr)(unsafe.Pointer(argv + uintptr(libc.PreIncInt32(&i, 1))*4)))
			} else if libc.Xstrcmp(tls, z, ts+23656) == 0 {
				doExclusive = 1
			} else if libc.Xstrcmp(tls, z, ts+23666) == 0 {
				doFullFSync = 1
			} else if libc.Xstrcmp(tls, z, ts+23676) == 0 {
				g.doCheckpoint = 1
			} else if libc.Xstrcmp(tls, z, ts+23687) == 0 {
				g.bSqlOnly = 1
				g.bExplain = 1
			} else if libc.Xstrcmp(tls, z, ts+23695) == 0 {
				if i >= argc-2 {
					fatal_error(tls, ts+23632, libc.VaList(bp+24, *(*uintptr)(unsafe.Pointer(argv + uintptr(i)*4))))
				}
				...
				i = i + 2
			} else if libc.Xstrcmp(tls, z, ts+23700) == 0 {
				doIncrvac = 1
			} else if libc.Xstrcmp(tls, z, ts+23711) == 0 {
				if i >= argc-1 {
					fatal_error(tls, ts+23632, libc.VaList(bp+32, *(*uintptr)(unsafe.Pointer(argv + uintptr(i)*4))))
				}
				zJMode = *(*uintptr)(unsafe.Pointer(argv + uintptr(libc.PreIncInt32(&i, 1))*4))
			} else if libc.Xstrcmp(tls, z, ts+23719) == 0 {
				if i >= argc-1 {
					fatal_error(tls, ts+23632, libc.VaList(bp+40, *(*uintptr)(unsafe.Pointer(argv + uintptr(i)*4))))
				}
				zKey = *(*uintptr)(unsafe.Pointer(argv + uintptr(libc.PreIncInt32(&i, 1))*4))
			} else if libc.Xstrcmp(tls, z, ts+23723) == 0 {
				if i >= argc-2 {
					fatal_error(tls, ts+23632, libc.VaList(bp+48, *(*uintptr)(unsafe.Pointer(argv + uintptr(i)*4))))
				}
				...
				i = i + 2
			} else if libc.Xstrcmp(tls, z, ts+23733) == 0 {
				memDb = 1
			} else if libc.Xstrcmp(tls, z, ts+23739) == 0 {
				sqlite3.Xsqlite3_config(tls, SQLITE_CONFIG_MULTITHREAD, 0)
			} else if libc.Xstrcmp(tls, z, ts+23751) == 0 {
				sqlite3.Xsqlite3_config(tls, SQLITE_CONFIG_MEMSTATUS, libc.VaList(bp+56, 0))
			} else if libc.Xstrcmp(tls, z, ts+23761) == 0 {
				if i >= argc-1 {
					fatal_error(tls, ts+23632, libc.VaList(bp+64, *(*uintptr)(unsafe.Pointer(argv + uintptr(i)*4))))
				}
				mmapSize = integerValue(tls, *(*uintptr)(unsafe.Pointer(argv + uintptr(libc.PreIncInt32(&i, 1))*4)))
			} else if libc.Xstrcmp(tls, z, ts+23766) == 0 {
				openFlags = openFlags | SQLITE_OPEN_NOMUTEX
			} else if libc.Xstrcmp(tls, z, ts+23774) == 0 {
				noSync = 1
			} else if libc.Xstrcmp(tls, z, ts+23781) == 0 {
				g.zNN = ts + 23789
			} else if libc.Xstrcmp(tls, z, ts+23798) == 0 {
				if i >= argc-1 {
					fatal_error(tls, ts+23632, libc.VaList(bp+72, *(*uintptr)(unsafe.Pointer(argv + uintptr(i)*4))))
				}
				i++
				if libc.Xstrcmp(tls, *(*uintptr)(unsafe.Pointer(argv + uintptr(i)*4)), ts+23805) == 0 {
					g.hashFile = libc.Xstdout
				} else {
					g.hashFile = libc.Xfopen(tls, *(*uintptr)(unsafe.Pointer(argv + uintptr(i)*4)), ts+23807)
					if g.hashFile == uintptr(0) {
						fatal_error(tls, ts+23810, libc.VaList(bp+80, *(*uintptr)(unsafe.Pointer(argv + uintptr(i)*4))))
					}
				}
			} else if libc.Xstrcmp(tls, z, ts+23840) == 0 {
				if i >= argc-1 {
					fatal_error(tls, ts+23632, libc.VaList(bp+88, *(*uintptr)(unsafe.Pointer(argv + uintptr(i)*4))))
				}
				pageSize = integerValue(tls, *(*uintptr)(unsafe.Pointer(argv + uintptr(libc.PreIncInt32(&i, 1))*4)))
			} else if libc.Xstrcmp(tls, z, ts+23849) == 0 {
				if i >= argc-2 {
					fatal_error(tls, ts+23632, libc.VaList(bp+96, *(*uintptr)(unsafe.Pointer(argv + uintptr(i)*4))))
				}
				...
				doPCache = 1
				i = i + 2
			} else if libc.Xstrcmp(tls, z, ts+23856) == 0 {
				g.zPK = ts + 23867
			} else if libc.Xstrcmp(tls, z, ts+23879) == 0 {
				if i >= argc-1 {
					fatal_error(tls, ts+23632, libc.VaList(bp+104, *(*uintptr)(unsafe.Pointer(argv + uintptr(i)*4))))
				}
				g.nRepeat = integerValue(tls, *(*uintptr)(unsafe.Pointer(argv + uintptr(libc.PreIncInt32(&i, 1))*4)))
			} else if libc.Xstrcmp(tls, z, ts+23886) == 0 {
				g.bReprepare = 1
			} else if libc.Xstrcmp(tls, z, ts+23896) == 0 {
				sqlite3.Xsqlite3_config(tls, SQLITE_CONFIG_SERIALIZED, 0)
			} else if libc.Xstrcmp(tls, z, ts+23907) == 0 {
				sqlite3.Xsqlite3_config(tls, SQLITE_CONFIG_SINGLETHREAD, 0)
			} else if libc.Xstrcmp(tls, z, ts+23920) == 0 {
				if i >= argc-1 {
					fatal_error(tls, ts+23632, libc.VaList(bp+112, *(*uintptr)(unsafe.Pointer(argv + uintptr(i)*4))))
				}
				if g.pScript != 0 {
					libc.Xfclose(tls, g.pScript)
				}
				g.pScript = libc.Xfopen(tls, *(*uintptr)(unsafe.Pointer(argv + uintptr(libc.PreIncInt32(&i, 1))*4)), ts+23807)
				if g.pScript == uintptr(0) {
					fatal_error(tls, ts+23927, libc.VaList(bp+120, *(*uintptr)(unsafe.Pointer(argv + uintptr(i)*4))))
				}
			} else if libc.Xstrcmp(tls, z, ts+23960) == 0 {
				g.bSqlOnly = 1
			} else if libc.Xstrcmp(tls, z, ts+23968) == 0 {
				g.bMemShrink = 1
			} else if libc.Xstrcmp(tls, z, ts+23982) == 0 {
				if i >= argc-1 {
					fatal_error(tls, ts+23632, libc.VaList(bp+128, *(*uintptr)(unsafe.Pointer(argv + uintptr(i)*4))))
				}
				g.szTest = integerValue(tls, *(*uintptr)(unsafe.Pointer(argv + uintptr(libc.PreIncInt32(&i, 1))*4)))
			} else if libc.Xstrcmp(tls, z, ts+23987) == 0 {
				showStats = 1
			} else if libc.Xstrcmp(tls, z, ts+23993) == 0 {
				if i >= argc-1 {
					fatal_error(tls, ts+23632, libc.VaList(bp+136, *(*uintptr)(unsafe.Pointer(argv + uintptr(i)*4))))
				}
				i++
				if int32(*(*uint8)(unsafe.Pointer(*(*uintptr)(unsafe.Pointer(argv + uintptr(i)*4))))) < '0' || int32(*(*uint8)(unsafe.Pointer(*(*uintptr)(unsafe.Pointer(argv + uintptr(i)*4))))) > '9' || int32(*(*uint8)(unsafe.Pointer(*(*uintptr)(unsafe.Pointer(argv + uintptr(i)*4)) + 1))) != 0 {
					fatal_error(tls, ts+23998, 0)
				}
				g.eTemp = int32(*(*uint8)(unsafe.Pointer(*(*uintptr)(unsafe.Pointer(argv + uintptr(i)*4))))) - '0'
			} else if libc.Xstrcmp(tls, z, ts+24051) == 0 {
				if i >= argc-1 {
					fatal_error(tls, ts+23632, libc.VaList(bp+144, *(*uintptr)(unsafe.Pointer(argv + uintptr(i)*4))))
				}
				zTSet = *(*uintptr)(unsafe.Pointer(argv + uintptr(libc.PreIncInt32(&i, 1))*4))
			} else if libc.Xstrcmp(tls, z, ts+24059) == 0 {
				doTrace = 1
			} else if libc.Xstrcmp(tls, z, ts+24065) == 0 {
				if i >= argc-1 {
					fatal_error(tls, ts+23632, libc.VaList(bp+152, *(*uintptr)(unsafe.Pointer(argv + uintptr(i)*4))))
				}
				nThread = integerValue(tls, *(*uintptr)(unsafe.Pointer(argv + uintptr(libc.PreIncInt32(&i, 1))*4)))
			} else if libc.Xstrcmp(tls, z, ts+24073) == 0 {
				zEncoding = ts + 24073
			} else if libc.Xstrcmp(tls, z, ts+24081) == 0 {
				zEncoding = ts + 24081
			} else if libc.Xstrcmp(tls, z, ts+24089) == 0 {
				g.bVerify = 1
				HashInit(tls)
			} else if libc.Xstrcmp(tls, z, ts+24096) == 0 {
				if i >= argc-1 {
					fatal_error(tls, ts+23632, libc.VaList(bp+160, *(*uintptr)(unsafe.Pointer(argv + uintptr(i)*4))))
				}
				zVfs = *(*uintptr)(unsafe.Pointer(argv + uintptr(libc.PreIncInt32(&i, 1))*4))
			} else if libc.Xstrcmp(tls, z, ts+24100) == 0 {
				if i >= argc-1 {
					fatal_error(tls, ts+23632, libc.VaList(bp+168, *(*uintptr)(unsafe.Pointer(argv + uintptr(i)*4))))
				}
				g.nReserve = libc.Xatoi(tls, *(*uintptr)(unsafe.Pointer(argv + uintptr(libc.PreIncInt32(&i, 1))*4)))
			} else if libc.Xstrcmp(tls, z, ts+24108) == 0 {
				g.stmtScanStatus = 1
			} else if libc.Xstrcmp(tls, z, ts+24123) == 0 {
				if libc.Xstrstr(tls, g.zWR, ts+24137) != uintptr(0) {
				} else if libc.Xstrstr(tls, g.zWR, ts+24145) != uintptr(0) {
					g.zWR = ts + 24152
				} else {
					g.zWR = ts + 6454
				}
				g.zPK = ts + 23867
			} else if libc.Xstrcmp(tls, z, ts+24173) == 0 {
				if libc.Xstrstr(tls, g.zWR, ts+24145) != uintptr(0) {
				} else if libc.Xstrstr(tls, g.zWR, ts+24137) != uintptr(0) {
					g.zWR = ts + 24152
				} else {
					g.zWR = ts + 24145
				}
			} else if libc.Xstrcmp(tls, z, ts+24180) == 0 || libc.Xstrcmp(tls, z, ts+24185) == 0 {
				libc.Xprintf(tls, uintptr(unsafe.Pointer(&zHelp)), libc.VaList(bp+176, *(*uintptr)(unsafe.Pointer(argv))))
				libc.Xexit(tls, 0)
			} else {
				fatal_error(tls, ts+24187,
					libc.VaList(bp+184, *(*uintptr)(unsafe.Pointer(argv + uintptr(i)*4)), *(*uintptr)(unsafe.Pointer(argv))))
			}
		} else if zDbName == uintptr(0) {
			zDbName = *(*uintptr)(unsafe.Pointer(argv + uintptr(i)*4))
		} else {
			fatal_error(tls, ts+24228,
				libc.VaList(bp+200, *(*uintptr)(unsafe.Pointer(argv + uintptr(i)*4)), *(*uintptr)(unsafe.Pointer(argv))))
		}
	}
	if nHeap > 0 {
		pHeap = libc.Xmalloc(tls, uint32(nHeap))
		if pHeap == uintptr(0) {
			fatal_error(tls, ts+24271, libc.VaList(bp+216, nHeap))
		}
		rc = sqlite3.Xsqlite3_config(tls, SQLITE_CONFIG_HEAP, libc.VaList(bp+224, pHeap, nHeap, mnHeap))
		if rc != 0 {
			fatal_error(tls, ts+24301, libc.VaList(bp+248, rc))
		}
	}
	if doPCache != 0 {
		if nPCache > 0 && szPCache > 0 {
			...
			if pPCache == uintptr(0) {
				fatal_error(tls, ts+24332,
					libc.VaList(bp+256, int64(nPCache)*int64(szPCache)))
			}
		}
		rc = sqlite3.Xsqlite3_config(tls, SQLITE_CONFIG_PAGECACHE, libc.VaList(bp+264, pPCache, szPCache, nPCache))
		if rc != 0 {
			fatal_error(tls, ts+24366, libc.VaList(bp+288, rc))
		}
	}
	if nLook >= 0 {
	...
	if sqlite3.Xsqlite3_open_v2(tls, func() uintptr {
		...
		openFlags, zVfs) != 0 {
		fatal_error(tls, ts+24408, libc.VaList(bp+312, zDbName))
	}
	if nLook > 0 && szLook > 0 {
		...
		if rc != 0 {
			fatal_error(tls, ts+24439, libc.VaList(bp+344, rc))
		}
	}
	if g.nReserve > 0 {
		sqlite3.Xsqlite3_file_control(tls, g.db, uintptr(0), SQLITE_FCNTL_RESERVE_BYTES, uintptr(unsafe.Pointer(&g))+64)
	}
	if g.stmtScanStatus != 0 {
		sqlite3.Xsqlite3_db_config(tls, g.db, SQLITE_DBCONFIG_STMT_SCANSTATUS, libc.VaList(bp+352, 1, 0))
	}

	sqlite3.Xsqlite3_create_function(tls, g.db, ts+24475, 0, SQLITE_UTF8, uintptr(0), *(*uintptr)(unsafe.Pointer(&struct {
	...
	if memDb > 0 {
		speedtest1_exec(tls, ts+24482, 0)
	}
	if mmapSize > 0 {
		speedtest1_exec(tls, ts+24507, libc.VaList(bp+368, mmapSize))
	}
	speedtest1_exec(tls, ts+24527, libc.VaList(bp+376, nThread))
	if zKey != 0 {
		speedtest1_exec(tls, ts+24545, libc.VaList(bp+384, zKey))
	}
	if zEncoding != 0 {
		speedtest1_exec(tls, ts+24562, libc.VaList(bp+392, zEncoding))
	}
	if doAutovac != 0 {
		speedtest1_exec(tls, ts+24581, 0)
	} else if doIncrvac != 0 {
		speedtest1_exec(tls, ts+24605, 0)
	}
	if pageSize != 0 {
		speedtest1_exec(tls, ts+24636, libc.VaList(bp+400, pageSize))
	}
	if cacheSize != 0 {
		speedtest1_exec(tls, ts+24656, libc.VaList(bp+408, cacheSize))
	}
	if noSync != 0 {
		speedtest1_exec(tls, ts+24677, 0)
	} else if doFullFSync != 0 {
		speedtest1_exec(tls, ts+24700, 0)
	}
	if doExclusive != 0 {
		speedtest1_exec(tls, ts+24720, 0)
	}
	if zJMode != 0 {
		speedtest1_exec(tls, ts+24750, libc.VaList(bp+416, zJMode))
	}

	if g.bExplain != 0 {
		libc.Xprintf(tls, ts+24773, 0)
	}
	for __ccgo1 := true; __ccgo1; __ccgo1 = *(*uint8)(unsafe.Pointer(zTSet)) != 0 {
		...
		if g.iTotal > int64(0) || zComma != uintptr(0) {
			libc.Xprintf(tls, ts+24792, libc.VaList(bp+424, zThisTest))
		}
		if libc.Xstrcmp(tls, zThisTest, ts+23547) == 0 {
			testset_main(tls)
		} else if libc.Xstrcmp(tls, zThisTest, ts+24819) == 0 {
			testset_debug1(tls)
		} else if libc.Xstrcmp(tls, zThisTest, ts+24826) == 0 {
			testset_orm(tls)
		} else if libc.Xstrcmp(tls, zThisTest, ts+24830) == 0 {
			testset_cte(tls)
		} else if libc.Xstrcmp(tls, zThisTest, ts+24834) == 0 {
			testset_fp(tls)
		} else if libc.Xstrcmp(tls, zThisTest, ts+24837) == 0 {
			testset_trigger(tls)
		} else if libc.Xstrcmp(tls, zThisTest, ts+24845) == 0 {
			testset_rtree(tls, 6, 147)
		} else {
			fatal_error(tls,
				ts+24851,
				libc.VaList(bp+432, zThisTest))
		}
		if *(*uint8)(unsafe.Pointer(zTSet)) != 0 {
			var zSql uintptr
			var zObj uintptr
			speedtest1_begin_test(tls, 999, ts+24920, 0)
			for 1 != 0 {
				...
				zSql = sqlite3.Xsqlite3_mprintf(tls, ts+25009, libc.VaList(bp+440, zObj))
			...
			for 1 != 0 {
				...
				zSql = sqlite3.Xsqlite3_mprintf(tls, ts+25009, libc.VaList(bp+448, zObj))
				speedtest1_exec(tls, zSql, 0)
				sqlite3.Xsqlite3_free(tls, zSql)
				sqlite3.Xsqlite3_free(tls, zObj)
			}
	...
	if showStats != 0 {
		sqlite3.Xsqlite3_exec(tls, g.db, ts+25100, *(*uintptr)(unsafe.Pointer(&struct {
			f func(*libc.TLS, uintptr, int32, uintptr, uintptr) int32
		}{xCompileOptions})), uintptr(0), uintptr(0))
	}

	if showStats != 0 {
		sqlite3.Xsqlite3_db_status(tls, g.db, SQLITE_DBSTATUS_LOOKASIDE_USED, bp+608, bp+612, 0)
		libc.Xprintf(tls, ts+25123, libc.VaList(bp+456, *(*int32)(unsafe.Pointer(bp + 608)), *(*int32)(unsafe.Pointer(bp + 612))))
		sqlite3.Xsqlite3_db_status(tls, g.db, SQLITE_DBSTATUS_LOOKASIDE_HIT, bp+608, bp+612, 0)
		libc.Xprintf(tls, ts+25168, libc.VaList(bp+472, *(*int32)(unsafe.Pointer(bp + 612))))
		sqlite3.Xsqlite3_db_status(tls, g.db, SQLITE_DBSTATUS_LOOKASIDE_MISS_SIZE, bp+608, bp+612, 0)
		libc.Xprintf(tls, ts+25204, libc.VaList(bp+480, *(*int32)(unsafe.Pointer(bp + 612))))
		sqlite3.Xsqlite3_db_status(tls, g.db, SQLITE_DBSTATUS_LOOKASIDE_MISS_FULL, bp+608, bp+612, 0)
		libc.Xprintf(tls, ts+25240, libc.VaList(bp+488, *(*int32)(unsafe.Pointer(bp + 612))))
		sqlite3.Xsqlite3_db_status(tls, g.db, SQLITE_DBSTATUS_CACHE_USED, bp+608, bp+612, 0)
		libc.Xprintf(tls, ts+25276, libc.VaList(bp+496, *(*int32)(unsafe.Pointer(bp + 608))))
		sqlite3.Xsqlite3_db_status(tls, g.db, SQLITE_DBSTATUS_CACHE_HIT, bp+608, bp+612, 1)
		libc.Xprintf(tls, ts+25318, libc.VaList(bp+504, *(*int32)(unsafe.Pointer(bp + 608))))
		sqlite3.Xsqlite3_db_status(tls, g.db, SQLITE_DBSTATUS_CACHE_MISS, bp+608, bp+612, 1)
		libc.Xprintf(tls, ts+25354, libc.VaList(bp+512, *(*int32)(unsafe.Pointer(bp + 608))))
		sqlite3.Xsqlite3_db_status(tls, g.db, SQLITE_DBSTATUS_CACHE_WRITE, bp+608, bp+612, 1)
		libc.Xprintf(tls, ts+25390, libc.VaList(bp+520, *(*int32)(unsafe.Pointer(bp + 608))))
		sqlite3.Xsqlite3_db_status(tls, g.db, SQLITE_DBSTATUS_SCHEMA_USED, bp+608, bp+612, 0)
		libc.Xprintf(tls, ts+25426, libc.VaList(bp+528, *(*int32)(unsafe.Pointer(bp + 608))))
		sqlite3.Xsqlite3_db_status(tls, g.db, SQLITE_DBSTATUS_STMT_USED, bp+608, bp+612, 0)
		libc.Xprintf(tls, ts+25468, libc.VaList(bp+536, *(*int32)(unsafe.Pointer(bp + 608))))
	}

	sqlite3.Xsqlite3_close(tls, g.db)

	if showStats != 0 {
		sqlite3.Xsqlite3_status(tls, SQLITE_STATUS_MEMORY_USED, bp+608, bp+612, 0)
		libc.Xprintf(tls, ts+25510, libc.VaList(bp+544, *(*int32)(unsafe.Pointer(bp + 608)), *(*int32)(unsafe.Pointer(bp + 612))))
		sqlite3.Xsqlite3_status(tls, SQLITE_STATUS_MALLOC_COUNT, bp+608, bp+612, 0)
		libc.Xprintf(tls, ts+25555, libc.VaList(bp+560, *(*int32)(unsafe.Pointer(bp + 608)), *(*int32)(unsafe.Pointer(bp + 612))))
		sqlite3.Xsqlite3_status(tls, SQLITE_STATUS_PAGECACHE_OVERFLOW, bp+608, bp+612, 0)
		libc.Xprintf(tls, ts+25600, libc.VaList(bp+576, *(*int32)(unsafe.Pointer(bp + 608)), *(*int32)(unsafe.Pointer(bp + 612))))
		sqlite3.Xsqlite3_status(tls, SQLITE_STATUS_MALLOC_SIZE, bp+608, bp+612, 0)
		libc.Xprintf(tls, ts+25645, libc.VaList(bp+592, *(*int32)(unsafe.Pointer(bp + 612))))
		sqlite3.Xsqlite3_status(tls, SQLITE_STATUS_PAGECACHE_SIZE, bp+608, bp+612, 0)
		libc.Xprintf(tls, ts+25687, libc.VaList(bp+600, *(*int32)(unsafe.Pointer(bp + 612))))
	}
