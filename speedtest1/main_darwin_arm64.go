...
type rusage_info_v6 = struct {
	...
	ri_runnable_time                 uint64
	ri_flags                         uint64
	ri_user_ptime                    uint64
	ri_system_ptime                  uint64
	ri_pinstructions                 uint64
	ri_pcycles                       uint64
	ri_energy_nj                     uint64
	ri_penergy_nj                    uint64
	ri_reserved                      [14]uint64
}
...
func _OSSwapInt16(tls *libc.TLS, _data uint16) uint16 {
	return uint16(int32(_data)<<8 | int32(_data)>>8)
}

func _OSSwapInt32(tls *libc.TLS, _data uint32) uint32 {
	_data = libc.X__builtin_bswap32(tls, _data)

	return _data
}

func _OSSwapInt64(tls *libc.TLS, _data uint64) uint64 {
	return libc.X__builtin_bswap64(tls, _data)
}

type _OSUnalignedU16 = struct{ __val uint16 }

type _OSUnalignedU32 = struct{ __val uint32 }

type _OSUnalignedU64 = struct{ __val uint64 }
...
func __darwin_check_fd_set(tls *libc.TLS, _a int32, _b uintptr) int32 {
	if uint64(uintptr(unsafe.Pointer(&__darwin_check_fd_set_overflow))) != uint64(0) {
		return (*struct {
	...
	for sqlite3.Xsqlite3_step(tls, g.pStmt) == SQLITE_ROW {
		...
		for i = 0; i < n; i++ {
			...
			if uint64(g.nResult+len) < uint64(unsafe.Sizeof([3000]int8{}))-uint64(2) {
				if g.nResult > 0 {
					*(*int8)(unsafe.Pointer(uintptr(unsafe.Pointer(&g)) + 132 + uintptr(libc.PostIncInt32(&g.nResult, 1)))) = int8(' ')
				}
				libc.X__builtin___memcpy_chk(tls, uintptr(unsafe.Pointer(&g))+132+uintptr(g.nResult), z, uint64(len+1), libc.X__builtin_object_size(tls, uintptr(unsafe.Pointer(&g))+132+uintptr(g.nResult), 0))
